...
export default function Thanks() {
  ...
  const mode = router.query.mode; // "meeting" or "only" or undefined

  return (
    <div className="min-h-screen bg-[#faf7f2] flex flex-col items-center justify-center px-4 relative">
      {/* ロゴ */}
      <div className="w-full max-w-lg flex flex-col items-center pt-12 mb-6">
        <Image src="/logo.png" alt="ロゴ" width={180} height={100} priority />
      </div>
      {/* 本文 */}
      <div className="w-full max-w-lg bg-white rounded-3xl shadow-xl py-10 px-6 flex flex-col items-center">
        <div className="text-3xl md:text-4xl font-extrabold text-[#223a50] mb-4 text-center">
          ご回答ありがとうございました！
        </div>
        {/* モードによる案内切り替え */}
        <div className="text-lg text-gray-700 text-center mb-6 leading-relaxed font-semibold">
          回答内容が送信されました。<br />
          {mode === "meeting" && (
            <>「分析結果＋面談」を選択いただきました。<br />スタッフよりLINEやDMでご連絡します！</>
          )}
          {mode === "only" && (
            <>「分析結果のみ」を選択いただきました。<br />フィードバックはLINEやDMでお送りします。</>
          )}
          {!mode && (
