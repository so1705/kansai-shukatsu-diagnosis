...
// 各種リスト（省略部分はそのまま続けてください）
const years = Array.from({ length: 30 }, (_, i) => `${1995 + i}年`);
const months = Array.from({ length: 12 }, (_, i) => `${i + 1}月`);
const days = Array.from({ length: 31 }, (_, i) => `${i + 1}日`);
const genders = ["男性", "女性", "その他"];
const prefectures = [ "希望無し","北海道", "青森県", "岩手県", "宮城県", "秋田県", "山形県", "福島県", "茨城県", "栃木県", "群馬県", "埼玉県", "千葉県", "東京都", "神奈川県", "新潟県", "富山県", "石川県", "福井県", "山梨県", "長野県", "岐阜県", "静岡県", "愛知県", "三重県", "滋賀県", "京都府", "大阪府", "兵庫県", "奈良県", "和歌山県", "鳥取県", "島根県", "岡山県", "広島県", "山口県", "徳島県", "香川県", "愛媛県", "高知県", "福岡県", "佐賀県", "長崎県", "熊本県", "大分県", "宮崎県", "鹿児島県", "沖縄県"
];
...
const faculties = [ "人文科学系", "社会科学系", "理工系", "生命・医療・健康系",
...
export default function IndexPage() {
  ...
  const validate = () => {
    const requiredFields = ["instagram", "line", "lastName", "firstName", "birthYear", "birthMonth", "birthDay", "gender", "address", "phone", "email", "university", "faculty", "grade", "graduateYear", "graduateMonth", "industry1", "job1", "location1"];
  ...
  const select = (label, key, list, optional = false) => (
    <div className="mb-4 text-left w-full max-w-md mx-auto">
  ...
  return (
    <div className="min-h-screen bg-[#fdf8f3] flex flex-col items-center justify-center px-4 py-10">
      <div className="w-full max-w-2xl flex flex-col items-center mx-auto px-4 text-left">
        <Image src="/logo.png" alt="ロゴ" width={180} height={100} className="mb-4" />
        <div className="text-xl font-bold text-[#223a50] mb-4 text-center">
          より正確な自己分析結果のために<br />下記詳細の入力をお願いします！
          <br />
          <br />
        </div>
        ...
        <div className="mb-6 text-sm">
          <label className="block font-semibold mb-2">
            フィードバックは公式ラインでさせていただきます！<br />
            本人確認の為にお手数ですがご記入ください！<br />
            （最後のページに追加URLがあります）
          </label>
        </div>
        ...
        <button onClick={handleSubmit} className="w-full max-w-md py-4 bg-blue-600 text-white font-bold rounded-lg mt-6 mx-auto">
          診断スタート！
        </button>
      </div>
    </div>
